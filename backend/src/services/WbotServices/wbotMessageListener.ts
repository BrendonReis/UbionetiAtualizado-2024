...
const handleChartbot = async (ticket: Ticket, msg: WAMessage, wbot: Session, dontReadTheFirstQuestion: boolean = false) => {
  ...
  console.log("Queue encontrada:", queue);

  const messageBody = getBodyMessage(msg);
  console.log("Mensagem recebida:", messageBody);

  if (messageBody == "00") {
    console.log("Mensagem 00 recebida, voltando para o menu inicial");
  ...
  if (!isNil(queue) && !isNil(ticket.queueOptionId) && messageBody == "0") {
    console.log("Mensagem 0 recebida, voltando para o menu anterior");
    const option = await QueueOption.findByPk(ticket.queueOptionId);
    await ticket.update({ queueOptionId: option?.parentId });
  } else if (!isNil(queue) && !isNil(ticket.queueOptionId)) {
    console.log("Escolhendo uma opção");
    ...
    console.log("Respondendo a primeira pergunta");
  ...
  await ticket.reload();
  console.log("Ticket recarregado:", ticket);

  if (!isNil(queue) && isNil(ticket.queueOptionId)) {
    console.log("Sem opção de fila selecionada, listando opções");
    ...
    console.log("Opções de fila disponíveis:", queueOptions);
    ...
    console.log("Configuração do chatbot:", buttonActive);
    ...
    const botButton = async () => {
      ...
      console.log("Enviando mensagem com botões:", buttons);
    ...
    const botText = async () => {
      ...
      console.log("Enviando mensagem de texto com opções:", options);
    ...
    if (buttonActive && buttonActive.value) {
      if (buttonActive.value === "button" && queueOptions.length <= 4) {
        console.log("Usando botões para mostrar as opções");
        return botButton();
      } else if (buttonActive.value === "text") {
        console.log("Usando texto para mostrar as opções");
        return botText();
      } else if (buttonActive.value === "button" && queueOptions.length > 4) {
        console.log("Muitas opções, usando texto em vez de botões");
        return botText();
      }
    } else {
      console.log("Configuração do chatbot não encontrada ou valor inválido.");
    }
  } else if (!isNil(queue) && !isNil(ticket.queueOptionId)) {
    console.log("Opção de fila selecionada, carregando opções subsequentes");
    ...
    console.log("Opções subsequentes encontradas:", queueOptions);
    ...
    if (queueOptions.length > -1) {
      ...
      console.log("Configuração do chatbot para opções subsequentes:", buttonActive);
      ...
      const botList = async () => {
        ...
        console.log("Enviando mensagem com lista de opções:", sectionsRows);
      ...
      const botButton = async () => {
        ...
        console.log("Enviando mensagem com botões subsequentes:", buttons);
      ...
      const botText = async () => {
        ...
        console.log("Enviando mensagem de texto com opções subsequentes:", options);
